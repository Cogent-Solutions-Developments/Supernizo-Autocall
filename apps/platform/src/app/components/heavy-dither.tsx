...
// Shared dither shader from Supernizo Heavy (components/ui/dither.tsx).
...
vec3 dither(vec2 uv, vec3 color) {
  ...
  float sourceMax = max(max(color.r, color.g), color.b);
  if (sourceMax <= 0.001) return vec3(0.0);
  vec3 hue = color / sourceMax;
  float brightness = clamp(sourceMax * 2.0 + threshold * step, 0.0, 1.0);
  float quantized = floor(brightness * (colorNum - 1.0) + 0.5) / (colorNum - 1.0);
  return hue * quantized * 0.66;
}
...
class RetroEffectImpl extends Effect {
  public override uniforms: Map<string, THREE.Uniform<number>>;
...
function DitherFrameLimiter({ active, maxFps }: { active: boolean; maxFps: number }) {
  ...
  useEffect(() => {
    ...
    const intervalMs = Math.max(1000 / Math.max(maxFps, 1), 16);
...
export default function Dither({
  ...
  return (
    ...
    >
      <DitherFrameLimiter active={!disableAnimation} maxFps={maxFps} />
      <DitheredWaves
        waveSpeed={waveSpeed}
