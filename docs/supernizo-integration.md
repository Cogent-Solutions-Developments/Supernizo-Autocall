...
7. Supernizo administrators receive Autocall `ADMIN`; other authorized staff receive `AGENT`. Every eligible signed-in user can access all active events without per-event assignment. Administrators retain event management and inactive-event access.
...
Open Light and Heavy tabs refresh their current user every 15 seconds and on focus, visibility restoration and reconnection. A rejected current token clears the portal session; a refreshed permission list updates navigation. Autocall checks its live server session on the same schedule and immediately reacts to HTTP 401 responses from dashboard API requests. Rejection removes the dashboard subtree, cleaning up its browser subscriptions and media through the existing unmount handlers. Network failures and server outages do not erase a login. Browser scheduling and request latency affect detection time; this does not change server-side SSE or LiveKit token lifetimes. New subscriptions and protected actions continue to require fresh authorization.
...
4. Configure and deploy both frontends. Grant a test employee Autocall access, sign that employee in again, open Autocall once to provision the identity, and verify that all active events are available without assignment.
5. Verify both portals, a Supernizo administrator, a local Autocall administrator, an eligible agent with no memberships, an inactive account and a client account. Verify that eligible agents can access every active event while administrator APIs and inactive events remain forbidden. Verify expired/replayed codes, wrong state and logout/revocation in a second tab.
