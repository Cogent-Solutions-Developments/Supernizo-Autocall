...
5. Enable receiver `SUPERNIZO_DIRECTORY_SYNC_ENABLED=true`, then source `AUTOCALL_DIRECTORY_SYNC_ENABLED=true`. Restart/redeploy Autocall and the Supernizo API, default worker and Beat services so all load the same settings. Source delivery defaults to `AUTOCALL_PUBLIC_URL`; Autocall lookup uses `SUPERNIZO_BACKEND_URL`. Production requires canonical HTTPS URLs. Preserve the user's local port choices (Heavy 3000, Autocall 3001). For a Supernizo worker running in local Docker, keep `AUTOCALL_PUBLIC_URL=http://localhost:3001/autocall-db` for browser SSO and set `AUTOCALL_DIRECTORY_RECEIVER_URL=http://host.docker.internal:3001/autocall-db`, plus `AUTOCALL_ALLOW_LOCAL_HTTP=true`; the receiver override is accepted only outside production.
...
7. Verify eligible Supernizo users can open all active events without assignments. Verify revoked users cannot use protected routes, agents cannot open inactive events or administrator APIs, and existing event/call/chat records are unchanged.
...
Local development uses Heavy at `http://localhost:3000` and Autocall at `http://localhost:3001/autocall-db`. Browser SSO always uses `localhost`; a Supernizo Docker worker uses `host.docker.internal` only through `AUTOCALL_DIRECTORY_RECEIVER_URL` to reach the same Autocall process. Autocall's `pnpm dev` command explicitly selects port 3001. If Next.js reports another dev server already running for the same directory, use that server or stop it with Ctrl+C in its original terminal before restarting; launching another instance will fail.

Event assignments are retired. User creation, names, roles and Autocall eligibility remain managed in Supernizo. Every eligible signed-in user can access every active event, including newly created events. Administrators retain event management and inactive-event access. Protected requests still introspect Supernizo. The old assignment page redirects to Events; the old assignment APIs reject requests without changing data. Legacy membership rows are retained for compatibility but do not authorize access. No database migration or directory synchronization flag change is needed for this access policy.
...
Rollback: pause source delivery/receiver synchronization, retain tables/outbox/tombstones, and keep current SSO authorization. Do not roll back to pre-SSO code or re-enable local user creation. Protected use continues to require live source authorization. Re-enable and reconcile to recover; no identity/site data deletion is required.
