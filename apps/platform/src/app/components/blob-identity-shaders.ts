...
void main() {
  v_displacement = cnoise(position * 1.35 + vec3(u_time));
  v_normal = normalize(normalMatrix * normal);

  vec3 displaced_position = position + normal * (u_intensity * v_displacement);
...
void main() {
  vec3 light_direction = normalize(vec3(-0.45, 0.7, 1.0));
  float diffuse = max(dot(normalize(v_normal), light_direction), 0.0);
  float rim = pow(1.0 - abs(v_normal.z), 2.2);
  float surface = 0.025 + diffuse * 0.16 + rim * 0.09 + v_displacement * 0.035;
  vec3 color = vec3(clamp(surface, 0.015, 0.32));
