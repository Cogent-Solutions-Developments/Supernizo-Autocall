...
import { IncomingCallSummarySchema, type IncomingCallSummary } from '@supernizo/shared';

import { fetchAppApi } from '@/lib/app-fetch';

import { IncomingCallModal } from './incoming-call-modal';

const IncomingCallsResponseSchema = z.object({
  data: z.object({ calls: z.array(IncomingCallSummarySchema) }),
});

export function IncomingCallAlert({
  initialCalls,
}: Readonly<{ initialCalls: IncomingCallSummary[] }>) {
  const [calls, setCalls] = useState(initialCalls);
  const [selectedCall, setSelectedCall] = useState<IncomingCallSummary | null>(null);
    ...
    <>
        ...
        >
          ...
          <span className="min-w-0 flex-1">
            ...
            <span className="mt-1 block text-sm font-semibold text-strong">
              {call.call.type === 'VIDEO' ? 'Video call requested' : 'Voice call requested'}
            </span>
            <span className="mt-0.5 block text-sm text-muted">
              Open to accept and join securely.
            </span>
            <span className="mt-2 block space-y-0.5 text-xs text-muted">
              <span className="block truncate">Event: {call.eventName}</span>
              <span className="block truncate">Visitor location: {call.visitorLocation}</span>
            </span>
          </span>
        </button>
      ) : null}
      {selectedCall ? (
        <IncomingCallModal
          ...
          eventName={selectedCall.eventName}
