...
function SiteForm({
  ...
  return (
    <form className="grid gap-6" onSubmit={handleSubmit}>
      ...
      <details className="rounded-xl border border-slate-200 bg-slate-50 p-4" open={Boolean(defaultSite)}>
  ...
  return (
    <section className="grid gap-7">
      <section className="grid gap-6 lg:block">
        ...
        >
          <div className={`flex items-center py-2 ${isSidebarCollapsed ? 'justify-center' : 'justify-between px-3'}`}>
            {isSidebarCollapsed ? (
              <span className="grid size-9 place-items-center rounded-full bg-sky-300/15 text-xs font-bold text-sky-100">S</span>
            ) : (
              <div>
                <Image alt="Supernizo Autocall" className="h-auto w-44" priority src={supernizoLogo} />
                <div>
                  <p className="mt-4 text-xs font-bold tracking-[0.18em] text-sky-200 uppercase">Events</p>
                </div>
              </div>
            )}
          </div>
          ...
          {sites.length > 0 ? (
            <div className={`mt-5 grid min-h-0 flex-1 content-start gap-2 overflow-y-auto ${isSidebarCollapsed ? 'justify-items-center' : ''}`}>
                  ...
                  >
                    {isSidebarCollapsed ? (
                      <>
                        ...
                        <span className={`absolute -bottom-0.5 -right-0.5 size-2 rounded-full ring-2 ring-[#0b1a24] ${site.status === 'ACTIVE' ? 'bg-emerald-400' : 'bg-slate-300'}`} />
                      </>
                    ) : (
                      <span className="flex items-center gap-3">
                        <span className={`size-2 shrink-0 rounded-full ${site.status === 'ACTIVE' ? 'bg-emerald-400' : 'bg-slate-400'}`} />
            ...
            <p className="min-h-0 flex-1 px-3 pt-7 text-sm leading-6 text-white/75">No events have been registered yet.</p>
          )}
          {canManage ? (
            <div className={`mt-4 border-t border-white/15 pt-4 ${isSidebarCollapsed ? 'flex justify-center' : 'px-1'}`}>
              <button
        ...
        <div className="min-w-0">
          ...
          <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-7">
            <div className="border-b border-slate-100 pb-6">
              <p className="text-sm font-semibold tracking-[0.14em] text-blue-600 uppercase">New event</p>
              <h2 className="mt-2 text-2xl font-semibold tracking-tight text-slate-950">Register an event</h2>
          ...
          <div className="flex flex-wrap items-start justify-between gap-4 border-b border-slate-100 pb-6">
            <div>
              <div className="flex flex-wrap items-center gap-3">
                <p className="text-sm font-semibold tracking-[0.14em] text-blue-600 uppercase">Selected event</p>
                <EventStatus status={selectedSite.status} />
              </div>
              <h2 className="mt-2 text-2xl font-semibold tracking-tight text-slate-950">{selectedSite.name}</h2>
              <p className="mt-2 text-sm text-slate-600">Open the tools below or update this event’s website connection.</p>
            </div>
            {canManage ? (
              <button
          ...
          <div className="mt-6 grid gap-3 md:grid-cols-3">
            ...
            >
              ...
              <p className="mt-1 text-sm leading-5 text-slate-600">See who is active and respond in real time.</p>
            </Link>
            <Link
              className="rounded-2xl border border-slate-200 p-4 transition hover:border-blue-300 hover:bg-blue-50"
            ...
            >
              ...
              <p className="mt-1 text-sm leading-5 text-slate-600">Review call outcomes and missed-call reasons.</p>
            </Link>
            <Link
              className="rounded-2xl border border-slate-200 p-4 transition hover:border-blue-300 hover:bg-blue-50"
            ...
            >
              ...
              <p className="mt-1 text-sm leading-5 text-slate-600">Understand visitors, activity and campaigns.</p>
            </Link>
          </div>

          <div className="mt-6 rounded-2xl bg-slate-50 p-4">
            <p className="text-sm font-semibold text-slate-800">Tracker public key</p>
            <p className="mt-1 text-xs leading-5 text-slate-500">Use this key in the event website’s tracker snippet. It is safe to expose publicly.</p>
          ...
          {isEditing && canManage ? (
            <div className="mt-7 border-t border-slate-100 pt-7">
              <div className="mb-5">
                ...
                <p className="mt-1 text-sm text-slate-600">Changes apply only to {selectedSite.name}.</p>
              </div>
              <SiteForm
